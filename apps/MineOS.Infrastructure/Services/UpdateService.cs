...
public sealed class UpdateService : IUpdateService
{
    ...
    private async Task<Offers?> ComputeOffersAsync(Identity identity, CancellationToken cancellationToken)
    {
        ...
        var candidates = profiles
            ...
            .Select(p => ToOffer(p))
            // OfType rather than a null check: the check filters at run time but
            // leaves the element type nullable, so every consumer below took a
            // List<Offer?> where it declares List<Offer> (CS8620).
            .OfType<Offer>()
            .ToList();
